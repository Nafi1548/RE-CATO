...
class TurboHParams:
    ...
    # --- ADD CASMOPOLITAN DISCRETE LENGTH PARAMS ---
    length_init_discrete: int = 10      # Initial Hamming distance bounds
    length_min_discrete: int = 1        # Minimum Hamming distance 
    length_max_discrete: int = 67      # Maximum Hamming distance
    # -----------------------------------------------
    # --- MIXED SEARCH SPACE PARAMS ---
    ...
    def __post_init__(self):
        # # FIX BUG 12: Ensure failure_streak is defined
        # if getattr(self, "failure_streak", None) is None:
        #     # Safely calculate total dimensions from the configuration
        #     cat_len = len(getattr(self, "cat_dims", None) or getattr(self, "binary_dims", None) or [])
        #     cont_len = len(getattr(self, "cont_dims", None) or [])
        #     ord_len = len(getattr(self, "ordinal_dims", None) or [])
            
        #     total_dim = cat_len + cont_len + ord_len
            
        #     # Default to 4 (TuRBO standard) or the dimension count
        #     self.failure_streak = max(4, total_dim) if total_dim > 0 else 4
        # Safely calculate total dimensions from the mixed configuration
        ...
        # 1. Calibrate Failure Streak (Blend TuRBO and CASMOPOLITAN logic)
        if getattr(self, "failure_streak", None) is None:
            if total_dim > 0:
                # Use max(10, d//3) but cap it at CASMOPOLITAN's default of 40
                self.failure_streak = min(40, max(10, total_dim // 3))
            else:
                self.failure_streak = 4

        # 2. Calibrate Success Streak (Enable TR Expansion)
            # 1. Set default for success_streak only if it wasn't provided
        if self.success_streak is None:
            self.success_streak = 3
...
class TrustRegion(ABC, Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # --- ADD DISCRETE LENGTH BUFFER ---
        length_discrete = torch.tensor(
            tr_hparams.length_init_discrete, device=bounds.device, dtype=torch.long
        )
        self.register_buffer("length_discrete", length_discrete)
        # ----------------------------------
    ...
    def update_model(self) -> bool:
        ...
        if self._check_for_updates_to_model_training_data():
            ...
            # Extract cont_dims from your hyperparameters
            cont_indices = getattr(self.tr_hparams, "cont_dims", None)
            
            # Conditionally apply Normalize only if there are continuous dimensions
            ...
            if cont_indices is not None and len(cont_indices) == 0:
                ...
                if self.X_center is not None:
                    intf2 = Normalize(
                        d=self.dim, 
                        bounds=self.get_bounds(model_space=True), 
            ...
            self.model = get_fitted_model(
                ...
                cat_dims=self.tr_hparams.cat_dims,   # PASS CATEGORICAL DIMS
                cont_dims=self.tr_hparams.cont_dims, # PASS CONTINUOUS DIMS
        ...
        with torch.no_grad():
            ...
            for x_ in X.split(self.tr_hparams.pred_batch_limit):
                ...
                if sampler is not None:
                    ...
                    predictions.append(pm.squeeze(-2))
                    continue
    ...
    def update(
        ...
        **kwargs,
        ...
        if X_new is not None:
            ...
            if update_streaks:
                    ...
                    if self.n_successes >= self.tr_hparams.success_streak:
                        # Expand trust region (Continuous uses MORBO 2.0x)
                        self.length.fill_(
                            min(2.0 * self.length, self.tr_hparams.length_max)
                        )

                        # 2. PATCH: Apply discrete expansion (CASMOPOLITAN 1.5x alignment)
                        current_len = self.length_discrete.item()
                        # Use tr_multiplier (e.g., 1.5) to align with Casmopolitan's behavior
                        multiplier = getattr(self.tr_hparams, "tr_multiplier", 1.5)

                        new_len_discrete = min(
                            int(multiplier * current_len), 
                            self.tr_hparams.length_max_discrete
                        )
                        
                        # Safety catch: Prevent integer truncation from stalling expansion (e.g. int(1.5 * 1) == 1)
                        if new_len_discrete == int(current_len) and new_len_discrete < self.tr_hparams.length_max_discrete:
                            new_len_discrete += 1
                        ...
                        # Shrink trust region (Continuous uses MORBO / 2.0)
                        self.length.div_(2.0)

                        # --- SHRINK DISCRETE LENGTH (CASMOPOLITAN / 1.5) ---
                        new_len_discrete = int(self.length_discrete.item() / 1.5)
                        self.length_discrete.fill_(new_len_discrete)
                        # ---------------------------------------------------
                        self._reset_counters()
                        ...
                        # ADD THIS:
                        _cont_dims = getattr(self.tr_hparams, "cont_dims", None) or []
                        _bin_dims  = getattr(self.tr_hparams, "binary_dims", None) or []
                        if _bin_dims and not _cont_dims:
                            restart_cont = False
                        restart_disc = False

                        # Only trigger discrete restarts if the problem has discrete dimensions
                        # (Safely checking for either your new binary_dims or old cat_dims)
                        # if getattr(self.tr_hparams, "binary_dims", None) or getattr(self.tr_hparams, "cat_dims", None):
                        #     # CASMOPOLITAN TR dies when it hits the minimum discrete length (usually 1)
        ...
        if self.tr_hparams.verbose and X_new is not None:
            print(f"Num points in TR: {self.X.shape[0]}")
            length_disc = self.length_discrete.item() if hasattr(self, 'length_discrete') else "N/A"
            print(f"TR Lengths -> Cont: {self.length.item():.4f} | Disc: {length_disc}")
        return False
    ...
    def _update_training_data(self, X_all: Tensor, Y_all: Tensor) -> None:
        ...
        X_all_normalized = safe_normalize(
            X=X_all, 
            bounds=self.bounds, 
            indices=self.tr_hparams.cont_dims
        )
        ...
        if self.tr_hparams.trim_trace:
            tr_indices = get_indices_in_hypercube(
                ...
                # --- NEW ARGUMENTS ---
                # cat_dims=self.tr_hparams.cat_dims,
                cont_dims=self.tr_hparams.cont_dims,
                length_discrete=(self.length_discrete.item() * 2) if hasattr(self, 'length_discrete') else None,
                # --- PASSED NEW TOPOLOGY ---
                binary_dims=self.tr_hparams.binary_dims,
                ordinal_dims=self.tr_hparams.ordinal_dims,
                ordinal_config=self.tr_hparams.ordinal_config,
        ...
        # Inside _update_training_data where dists is calculated
        ...
        if binary_dims and not cont_dims:
            # FIX BUG 10: Use Hamming distance for pure discrete backfill
            hamming = ((X_all_normalized[:, binary_dims] - self.X_center_normalized[:, binary_dims]).abs() > 1e-6).sum(dim=1)
            ordinal_dist = 0
            if ordinal_dims:
                for idx, dim in enumerate(ordinal_dims):
                    # The max range for an ordinal dim is (ordinal_config[idx] - 1)
                    max_range = self.tr_hparams.ordinal_config[idx] - 1
                    
                    # Calculate raw absolute gap
                    raw_gap = (X_all_normalized[:, dim] - self.X_center_normalized[:, dim]).abs()
                    
                    # Normalize to [0, 1] so it matches the scale of a single binary flip
                    normalized_gap = raw_gap / max(1, max_range)
                    
                    ordinal_dist += normalized_gap
            dists = hamming.float() + ordinal_dist.float()
        else:
            dists = torch.cdist(X_all_normalized, self.X_center_normalized).squeeze()
    ...
    def get_bounds(self, model_space: bool = False):

        binary_dims = getattr(self.tr_hparams, "binary_dims", None)
        cont_dims = getattr(self.tr_hparams, "cont_dims", None)
        
        # FIX BUG 11: Pure discrete bounds
        if binary_dims and not cont_dims:
            bnds = torch.zeros((2, self.dim), device=self.bounds.device, dtype=self.bounds.dtype)
            bnds[1, binary_dims] = 1.0
            
            ordinal_dims = getattr(self.tr_hparams, "ordinal_dims", None)
            if ordinal_dims:
                for idx, dim in enumerate(ordinal_dims):
                    bnds[1, dim] = self.tr_hparams.ordinal_config[idx] - 1
            return bnds
        
        # ... fallback to original continuous L-inf bounds logic ...

        # normalized_X_center = normalize(self.X_center, bounds=self.bounds)
        normalized_X_center = safe_normalize(
            self.X_center, 
            bounds=self.bounds, 
            indices=self.tr_hparams.cont_dims
        )
...
class HypervolumeTrustRegion(TrustRegion):
    ...
    def _update_center_and_best_points(
        ...
        if X_center is not None:
            ...
            # pareto_X_normalized = safe_normalize(
            #     self.best_X, 
            #     bounds=self.bounds, 
            #     indices=self.tr_hparams.cont_dims
            # )

            # Ensure safe extraction of continuous dimensions
            cont_dims = getattr(self.tr_hparams, "cont_dims", None)

            # ---------------------------------------------------------
            # BUG 6 FIX: Prevent normalization of discrete/ordinal spaces
            # ---------------------------------------------------------
            if cont_dims is not None and len(cont_dims) > 0:
                ...
                # Pure discrete space: bypass normalization entirely
                pareto_X_normalized = self.best_X.clone()

            # ---------------------------------------------------------
            # TOPOLOGY EXTRACTION: Safely pull mixed-space config
            # ---------------------------------------------------------
            cont_dims = getattr(self.tr_hparams, "cont_dims", None)
            binary_dims = getattr(self.tr_hparams, "binary_dims", None)
            ordinal_dims = getattr(self.tr_hparams, "ordinal_dims", None)
            ordinal_config = getattr(self.tr_hparams, "ordinal_config", None)
            length_discrete = int(self.length_discrete.item()) if hasattr(self, 'length_discrete') else None
            ...
            if self.X_center_normalized is not None:
                ...
                # FIX PAPER ALIGNMENT ISSUE: Enforce Hamming boundaries for TR center anchoring
                indices_in_tr = get_indices_in_hypercube(
                    X_center=self.X_center_normalized, 
                    X=pareto_X_normalized, 
                    length=self.length,
                    # cat_dims=cat_dims,
                    cont_dims=cont_dims,
        ...
        if cont_dims is not None and len(cont_dims) > 0:
            ...
            # Pure discrete space: bypass normalization entirely
            self.X_center_normalized = self.X_center.clone()
