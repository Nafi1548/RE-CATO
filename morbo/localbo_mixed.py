...
from morbo.localbo_cat import CASMOPOLITANCat
...
from morbo.localbo_utils import train_gp
from morbo.localbo_utils import from_unit_cube, latin_hypercube, to_unit_cube
from morbo.localbo_utils import onehot2ordinal
from morbo.localbo_utils import random_sample_within_discrete_tr_ordinal
...
class CASMOPOLITANMixed(CASMOPOLITANCat):
    ...
    *Note*: in general, you should have the first d_cat dimensions as cat_dims, and the rest as the continuous dims.
    ...
    def _create_and_select_candidates(self, X, fX, length, n_training_steps, hypers, return_acq=False):
        ...
        #     X_cand = np.tile(x_center[0], (n_cand, 1))  # Initialize with correct global layout
        #     X_cand[:, self.cat_dims] = X_cand_cat       # Inject categorical mutations
        #     X_cand[:, self.cont_dims] = X_cand_cont     # Inject continuous mutations
        ...
        def thompson_interleaved_decoupled(n_restarts=3, steps=100):
            """
            Mathematically valid decoupled Thompson Sampling for mixed kernels.
            Uses an interleaved search over a fully frozen, spatially correlated
            posterior realization.
            """
            device = X_torch.device
            dtype = X_torch.dtype
            n_tr = X_torch.shape[0]

            # Extract base kernel components
            base_kern = gp.covar_module.base_kernel
            cat_kern = base_kern.categorical_kern
            cont_kern = base_kern.continuous_kern
            outputscale = gp.covar_module.outputscale.detach()

            # Helper to compute consistent Additive Kernel for TS
            def k_additive(A, B):
                k_c = cat_kern(A[:, self.cat_dims], B[:, self.cat_dims]).evaluate()
                k_x = cont_kern(A[:, self.cont_dims], B[:, self.cont_dims]).evaluate()
                return outputscale * (k_c + k_x)

            with torch.no_grad():
                # ----------------------------------------------------------------
                # 1. Exact Prior Draw at Training Points
                # ----------------------------------------------------------------
                # 1a. RFF prior for continuous dimensions (Matern-5/2)
                ls = cont_kern.lengthscale.detach().view(-1)
                n_rff = 512
                # Sample spectral frequencies from Student-T (df=5)
                W = torch.distributions.StudentT(df=5.0).sample((n_rff, len(self.cont_dims))).to(device, dtype) / ls
                b = torch.rand(n_rff, device=device, dtype=dtype) * 2 * np.pi
                
                def rff_features(X_c):
                    return np.sqrt(2.0 / n_rff) * torch.cos(X_c @ W.T + b)
                
                theta_rff = torch.randn(n_rff, device=device, dtype=dtype) * outputscale.sqrt()
                f_prior_cont_train = rff_features(X_torch[:, self.cont_dims]) @ theta_rff

                # 1b. Exact Cholesky prior for categorical dimensions
                K_cat_train = cat_kern(X_torch[:, self.cat_dims], X_torch[:, self.cat_dims]).evaluate() * outputscale
                L_cat = torch.linalg.cholesky(K_cat_train + 1e-6 * torch.eye(n_tr, device=device, dtype=dtype))
                z_cat = torch.randn(n_tr, device=device, dtype=dtype)
                f_prior_cat_train = L_cat @ z_cat

                # Combined training prior
                f_prior_train = f_prior_cont_train + f_prior_cat_train

                # 1c. Compute Correction Weights (Alpha) using Additive Kernel
                K_add_train = k_additive(X_torch, X_torch)
                noise = gp.likelihood.noise.detach().item()
                K_noisy = K_add_train + noise * torch.eye(n_tr, device=device, dtype=dtype)
                L_noisy = torch.linalg.cholesky(K_noisy)
                
                y_std = (y_torch - y_torch.mean()) / (y_torch.std() + 1e-8)
                residual = (y_std.view(-1) - f_prior_train).unsqueeze(-1)
                alpha = torch.cholesky_solve(residual, L_noisy).squeeze(-1)

                # Precompute K_cat^{-1} @ f_prior_cat_train for the Schur complement
                K_cat_inv_f_prior = torch.cholesky_solve(f_prior_cat_train.unsqueeze(-1), L_cat).squeeze(-1)

            # ----------------------------------------------------------------
            # 2. Setup Deterministic Z-Cache for Categorical Innovations
            # ----------------------------------------------------------------
            _z_cache = {}
            def get_z_cat(x_cat_np):
                key = tuple(x_cat_np.astype(int).tolist())
                if key not in _z_cache:
                    _z_cache[key] = torch.randn(1, device=device, dtype=dtype)
                return _z_cache[key]

            # ----------------------------------------------------------------
            # 3. Define the TS Landscape for Interleaved Search
            # ----------------------------------------------------------------
            def f_thompson(x):
                # Handle inputs from the optimizer cleanly
                if isinstance(x, np.ndarray):
                    x = torch.tensor(x, dtype=dtype, device=device)
                if x.dim() == 1:
                    x = x.unsqueeze(0)
                
                x_cont = x[:, self.cont_dims]
                x_cat = x[:, self.cat_dims].detach() # Gradients stop here

                # Continuous RFF Evaluation
                phi_x = rff_features(x_cont)
                f_prior_cont_x = (phi_x @ theta_rff).squeeze()

                # Categorical Schur Complement Evaluation
                k_cat_x = cat_kern(x_cat, X_torch[:, self.cat_dims]).evaluate().squeeze(0) * outputscale
                mean_cat_x = (k_cat_x @ K_cat_inv_f_prior).squeeze()
                
                k_cat_x_Kinv_k = (k_cat_x @ torch.cholesky_solve(k_cat_x.unsqueeze(-1), L_cat).squeeze()).squeeze()
                var_cat_x = torch.clamp(outputscale - k_cat_x_Kinv_k, min=1e-6)
                
                Z_cat = get_z_cat(x_cat.cpu().numpy().flatten())
                f_prior_cat_x = mean_cat_x + var_cat_x.sqrt() * Z_cat.squeeze()

                # Additive Correction Term
                k_x_train = k_additive(x, X_torch).squeeze(0)
                correction = (k_x_train @ alpha).squeeze()
                
                return (f_prior_cont_x + f_prior_cat_x + correction).squeeze()

            # ----------------------------------------------------------------
            # 4. Execute Multi-Start Interleaved Search
            # ----------------------------------------------------------------
            from .localbo_utils import interleaved_search
            ...
            # Generate batch by executing the search
            for b in range(self.batch_size):
                ...
                for _ in range(n_restarts):
                    ...
                    # Randomize starting position within trust regions
                    x0[self.cat_dims] = random_sample_within_discrete_tr_ordinal(
                        x0[self.cat_dims], self.length_discrete, self.config
                    )
                    x0[self.cont_dims] = lb + (ub - lb) * np.random.rand(len(self.cont_dims))
                    
                    x_opt, val = interleaved_search(
                        x_center=x0, f=f_thompson,
                        cat_dims=self.cat_dims, cont_dims=self.cont_dims,
                        config=self.config, ub=ub, lb=lb,
                        max_hamming_dist=self.length_discrete,
                        n_restart=1, batch_size=1, interval=1, step=steps
                    )
                ...
                X_next[b, :] = best_x.flatten()
                y_next[b, :] = best_val  # Store the TS surrogate value
        ...
        #     X_next, acq_next = thompson()
            ...
            X_next, acq_next = thompson_interleaved_decoupled(n_restarts=3, steps=100)
