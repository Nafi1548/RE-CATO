...
# Import the test function from the uploaded file
from mixed_test_func.synthetic import Func2C

def main():
    parser = argparse.ArgumentParser(description='Run Func2C on merged MORBO+Casmopolitan framework')
    parser.add_argument('--max_iters', type=int, default=50, help='Maximum number of BO iterations.')
    ...
    f_func2C = Func2C(lamda=args.lamda)
    cat_dims = list(f_func2C.categorical_dims)
    cont_dims = list(f_func2C.continuous_dims)
    config = list(f_func2C.config)
    dim = f_func2C.dim  # Total 4 dimensions (2 cat, 2 cont)

    # Wrapper to make Func2C compatible with MORBO 
    # MORBO maximizes, but Func2C is a minimization problem
    def eval_objective(X: torch.Tensor) -> torch.Tensor:
        X_np = X.cpu().detach().numpy()
        # compute() returns shape (N,)
        y_np = f_func2C.compute(X_np, normalize=False)
        y_tensor = torch.tensor(y_np, **tkwargs)
        return -y_tensor.view(-1, 1)  # Negate and reshape to (N, 1)

    # 3. Define Bounds for Mixed Space
    bounds = torch.zeros(2, dim, **tkwargs)
    # Continuous bounds [-1, 1]
    bounds[0, cont_dims] = torch.tensor(f_func2C.lb, **tkwargs)
    bounds[1, cont_dims] = torch.tensor(f_func2C.ub, **tkwargs)
    # Categorical bounds: 0 to (n_vertices - 1) -> [0, 2] and [0, 4]
    for i, c in enumerate(cat_dims):
        bounds[0, c] = 0
        bounds[1, c] = config[i] - 1  
    ...
    tr_hparams = TurboHParams(
        ...
        n_trust_regions=1,            # Single TR for Casmopolitan baseline
        max_tr_size=2000,
        min_tr_size=args.n_init,      
        hypervolume=False,            # Single objective
        use_ard=True,
        track_history=True,
        verbose=True
    )
    ...
    # Explicitly passing IdentityMCObjective() bypasses the strict num_outputs check in state.py
    trbo_state = TRBOState(
        dim=dim,
        max_evals=args.max_iters * args.batch_size + args.n_init,
        num_outputs=[1],             # Use list format to be perfectly safe
        num_objectives=1,
        bounds=bounds,
        tr_hparams=tr_hparams,
        objective=IdentityMCObjective()  # This stops the ValueError at line 204
    ).to(**tkwargs)
    ...
    # Ensure categorical dimensions are strictly integers
    ...
    print(f"\n----- Starting BO Loop for {args.max_iters} iterations -----")
    for i in range(args.max_iters):
        ...
        # Logging: Revert negation to display actual minimization values
        ...
        # Convert tensor to flat list for clean printing
        x_flat = X_next.flatten().cpu().tolist()
        x_str = "[" + ", ".join([f"{val:.4f}" for val in x_flat]) + "]"

        print(f"Iter {i:03d} | X: {x_str} | Last f(X): {current_y:.4f} | Best f(X): {best_y:.4f} | TR Length [Cont/Disc]: {trbo_state.trust_regions[0].length.item():.2f}/{trbo_state.trust_regions[0].length_discrete.item()}")

    # 9. Save Results
    save_file = os.path.join(args.save_path, f"morbo_casmo_func2C_seed{args.seed}.csv")
    res.to_csv(save_file, index=False)
    print(f"\nOptimization complete. Results saved to {save_file}")
