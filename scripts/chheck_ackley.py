...
def main():
    parser = argparse.ArgumentParser(description="Plot and compare Ackley53 BO performance.")
    parser.add_argument('--baseline', type=str, default='/home/nafi/CATO/casmopolitan/baseline_ackley.csv', 
                        help='Path to the Casmopolitan baseline CSV.')
    parser.add_argument('--merged', type=str, default='./output/morbo_casmo_ackley53_seed42.csv', 
                        help='Path to the merged MORBO+Casmopolitan CSV.')
    parser.add_argument('--save_plot', type=str, default='./output/ackley_comparison.png', 
    ...
    plt.title('Optimization Performance: Ackley53 (Mixed Space)', fontsize=14, fontweight='bold')
    plt.xlabel('Number of Function Evaluations', fontsize=12)
    plt.ylabel('Best Objective Value Found (Minimization)', fontsize=12)
    
    # Ackley minimum is 0, so a log scale often helps visualize late-stage convergence
    plt.yscale('log') 
